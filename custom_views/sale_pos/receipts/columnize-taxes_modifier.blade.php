				...
				@foreach($receipt_details->lines as $line)
					<tr style="border: 1px solid black; border-collapse: collapse">
                        ...
                        @endforeach
                         ...
                         @php
				$modifier_item_value = $modifier['unit_price_exc_tax'] / ( 1 + ( 15 / 100 ) ) ; 
			@endphp
                         
                          @php
				$modifier_item_actual_value = $modifier_item_value + $line['unit_price_before_discount'] ; 
			@endphp

                        @foreach($line['modifiers'] as $modifier)
                        <td style="border: 1px solid black; border-collapse: collapse;" class="text-right" valign="top">
							{{$line['unit_price_before_discount']}} </br> {{number_format($modifier_item_actual_value, 2)}}
						</td>
                        @endforeach
                                                   @foreach($line['modifiers'] as $modifier)
                        ...
                        <td style="border: 1px solid black; border-collapse: collapse;" class="text-right" valign="top">
							...
							@endphp
                                                    ...
                                                    @php
				                  $modifier_tax_value = $modifier['line_total'] - $modifier_item_value ; 
			                          @endphp
						@if(!empty($receipt_details->table_tax_headings))
						@foreach($receipt_details->table_tax_headings as $tax_heading)
							<td style="border: 1px solid black; border-collapse: collapse;" class="text-right word-wrap" valign="top">
								...
								@foreach($line['group_tax_details'] as $tax_detail)
									@if(strpos($tax_detail['name'], $tax_heading) !== FALSE)
										...
										</span> <br> <span> {{ $modifier_tax_value }} </span>
									@endif
								@endforeach
								@else
									...
									</span> <br>  <span> {{number_format($modifier_tax_value, 2)}} </span>
									@else NA
									@endif
								@endif
							</td>
