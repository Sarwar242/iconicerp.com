...
class TransactionUtil extends Util
{
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        ...
        // Calculating total price and total tax
        $price_exc_tax_total=0;
        $tax_total=0;
        ...
        if (in_array($transaction_type, ['sell', 'sales_order'])) {
                ...
                if (!empty($line['modifiers'])) {
                    foreach ($line['modifiers'] as $modifier){
                        if($this->num_uf($modifier['quantity'])>0){
                            $total_quantity += $modifier['quantity'];
                        }
                    }
                }
            }
            ...
            foreach ($details['lines'] as $line) {
                ...
                //For calculating price_excluding_tax . [needs to unformat the price]
                $price_exc_tax_total += $this->num_uf($line['unit_price_exc_tax'])*$this->num_uf((float)$line['quantity']);
                $tax_total += $this->num_uf($line['tax'])*$this->num_uf($line['quantity']);

                if($this->num_uf($line['quantity'])>0){
        ...
        $output['price_exc_tax_total']=$this->num_f($price_exc_tax_total, $show_currency, $business_details);
        $output['tax_total']=$this->num_f($tax_total, $show_currency, $business_details);
        $output['total_returned_items']=$total_returned_items;
        $output['total_quantity_returned']=$total_quantity_returned;
        //show cat code
        ...
        //New Labels
        $output['parent_label'] = 'Parent Sell:';
        // \Log::warning("parent: ".json_encode($transaction->return_parent_sell->invoice_no, JSON_PRETTY_PRINT));
        $output['parent_invoice_no']= !empty($transaction->return_parent_sell)? $transaction->return_parent_sell->invoice_no: 'N/A';
        $output['parent_transaction_date']= !empty($transaction->return_parent_sell)? $transaction->return_parent_sell->transaction_date: now();
        $output['table_unit_tax_label']= 'Tax';
        $output['total_amount_returned_wt_label']= 'Total Amount Returned (Without Tax)';
        $output['total_tax_returned_label']= 'Total Tax Returned';
        $output['total_item_returned_label']= 'Total Returned Items';
        $output['total_returned_qty_label']= 'Total Returned Quantity';
    ...
    protected function _receiptDetailsSellLines($lines, $il, $business_details)
    {
        ...
        foreach ($lines as $line) {
            ...
            if(!empty($tax_details)){
                $unit_tax = $line->unit_price_inc_tax/(1+($tax_details->amount/100));
			}
			else if(!empty($product) && !is_null($product->tax)){
            ...
            if (!empty($line->modifiers)) {
                foreach ($line->modifiers as $modifier_line) {
                    ...
                    \Log::warning("modifier_line: ". json_encode($modifier_line,JSON_PRETTY_PRINT));
                    \Log::warning("price_ex_tax : ". json_encode($price_ex_tax ,JSON_PRETTY_PRINT));
                    $modifier_line_array = [
                        //Field for 1st column
                        'name' => $product->name,
                        'variation' => (empty($variation->name) || $variation->name == 'DUMMY') ? '' : $variation->name,
                        ...
                        'price_exc_tax' => $modifier_line->quantity * $price_ex_tax,
						//<!--- Dev Changed -->
                        //Fields for 4th column
                        'line_total' => $modifier_line->unit_price_inc_tax * $modifier_line->quantity,
                    ];
            ...
            if (!empty($line->modifiers)) {
                foreach ($line->modifiers as $modifier_line) {
                    ...
                    $modifier_line_array = [
                        ...
                        'unit_price_inc_tax' => $modifier_line->unit_price_inc_tax,
                        'unit_price_exc_tax' => $price_ex_tax,
                        'price_exc_tax' =>  $modifier_line->quantity_returned * $price_ex_tax,      
                    ...
                    $line_array['line_total'] += $modifier_line->unit_price_inc_tax * $line->quantity_returned;
                }
            }
